...
def match_with_gaps(my_word, letters_guessed, other_word):  #function modified from homework
    '''
    ...
    #This step is not a requirement for Assignment 2
    ...
    for k in range(len(other_word)):
        if len(my_word) != len (other_word) or set(my_word) == {"_"}:
            break
        elif my_word[k] == "_":
            match_tally.append("")
...
def hangman_with_hints(secret_word):
    ...
    * If the guess is the symbol *, print out all words in wordlist that
    ...
    while guesses_remaining > 0:
        ...
        if hints_remaining > 0 and set(guessed_word_before) != {'_'}:
            hint_available = True
